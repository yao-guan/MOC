...
class Node(ABC):
    ...
    async def async_execute(self, input:Any, **kwargs):
        ...
        # Get neighbor summary if graph is provided
        neighbor_summary = ""
        graph = kwargs.get('graph', None)
        if graph is not None:
            try:
                query = input.get('task', '') if isinstance(input, dict) else str(input)
                neighbor_summary = await graph.get_neighbor_summary_with_ism(self.id, query)
            except Exception as e:
                print(f"Warning: Failed to get neighbor summary for node {self.id}: {e}")
                neighbor_summary = ""
        
        tasks = [asyncio.create_task(self._async_execute(input, spatial_info, temporal_info, neighbor_summary, **kwargs))]
